...
# S'assurer que le secret reel existe (copie depuis exemple si present)
if [ -f "../manifests/11-secret.yaml" ]; then
  echo "Utilisation de manifests/11-secret.yaml"
else
  echo "ERREUR: creez manifests/11-secret.yaml ->  partir de manifests/11-secret.example.yaml"
  exit 1
fi
